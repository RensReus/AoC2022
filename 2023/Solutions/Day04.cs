...
partial class Day04 : BaseDay
{
    ...
    public static int Part1(string input)
        => (int)ReadLines(input).Select(MatchCount).Sum(overlap => overlap == 0 ? 0 : Math.Pow(2, overlap - 1));

    private static int MatchCount(string v)
    {
        var match = LineRegex().Match(v);
        var winners = NumberRegex().Matches(match.Groups[1].Value).Select(x => x.Value);
        var actual = NumberRegex().Matches(match.Groups[2].Value).Select(x => x.Value);
        return winners.Intersect(actual).Count();
    }
    ...
    {
        var cards = ReadLines(input).Select(x => (MatchCount(x), 1)).ToList();
